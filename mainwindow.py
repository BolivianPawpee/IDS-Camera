...
class MainWindow(QMainWindow):
    ...
    def on_acquisition_timer(self):
        ...
        try:
            ...
            # Create a IDS peak IPL image from the buffer to facilitate
            # conversion to the `TARGET_PIXEL_FORMAT`.
            ipl_image = ids_peak_ipl_extension.BufferToImage(buffer)
            # NOTE: Use `ImageConverter` instead of `ConvertTo` because `ConvertTo`
            #       re-allocates conversion buffers on every call,
            #       which can be inefficient.
            #
            # Both `Convert` and `ConvertTo` guarantee that the image is at
            # least a deep copy, even if the source and target format match.
            ...
            self.__display.on_image_received(image)
            self.__display.update()
            # print(info)  # optional: see detections
            # --------------------------------
